...
public class EcdheEncryption {
    ...
    /**
     * runs the EC DH encryption in Ephemeral mode
     * the sender generates a key pair, encrypts with the senderPrivateKey and publicRecipientKey,
     * sends the senderPublicKey to the recipient and throws away his senderPrivateKey
     */
    ...
    public static EcdheModel encryptData(String senderKeyId, String recipientKeyId,  PublicKeyModel remotePublicKey, String deriveAlgorithm, String encryptionAlgorithm, byte[] dataToEncrypt) {
        // todo sanity checks
        ...
        String senderPublicKeyBase64 = base64EncodingNpe(senderPublicKey.getEncoded());
        EcdheModel ecdheModel = encryptAes(senderPublicKeyBase64, deriveAlgorithm, encryptionAlgorithm, transformation, senderKeyId, recipientKeyId, randomSalt32Byte, HKDF_KEY, encryptionKey, dataToEncrypt);
        return ecdheModel;
    }

    public static byte[] decryptData(EcdheModel encryptedData) {
        // todo sanity checks
        // todo remote public key = "EC", keyParameter allowed, encryptionAlgorithm allowed, dataToEncrypt != null
        ...
        // get public key
        String senderPublicKeyBase64 = encryptedData.getSenderPublicKeyBase64();
        PublicKey senderPublicKey;
        KeyFactory kf = null;
        try {
            kf = KeyFactory.getInstance("EC");
            senderPublicKey = (PublicKey) kf.generatePublic(new X509EncodedKeySpec(base64Decoding(senderPublicKeyBase64)));
        } catch (NoSuchAlgorithmException | InvalidKeySpecException e) {
            //throw new RuntimeException(e);
            Log.e(TAG, "Exception: " + e.getMessage());
            return null;
    ...
    public static EcdheModel encryptAes(String senderPublicKeyBase64, String deriveAlgorithm, String encryptionAlgorithm, String transformation, String alias, String aliasRecipient, byte[] deriveSalt, String deriveName, byte[] encryptionKey, byte[] data) {
        ...
        return new EcdheModel(aliasRecipient, senderPublicKeyBase64, base64EncodingNpe(deriveSalt), deriveName, deriveAlgorithm, encryptionAlgorithm, base64EncodingNpe(initVector), base64EncodingNpe(ciphertext));
    }
